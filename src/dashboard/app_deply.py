...
def load_model():
    ...
    explainer=joblib.load(os.path.join(base,"model","shap_explainer.pkl"))
...
def predict_direct(features:dict):
    ...
    df["Is_night"]=df((df["Hour"]>=22)|(df["Hour"]<=5)).astype(float)
    df=df.drop(columns=["Time","Amont"])
    ...
    }).reindex(pd.Series(shap_vals[0]).abs().sort_vaalues(ascending=False).index)
    return{
...
if st.button("Analyze Transaction",type="primary",use_container_width=True):
    ...
    with g_col:
        ...
        fig.upadate_layout(height=220,margin=dict(t=40,b=0,l=0,r=20))
        st.plotlly_chart(fig,use_container_width=True)
    ...
    with s_col:
        ...
        fig2=go.Figure(go.Bar(
            ...
            orientation="h",
            marker_color=["#e74c3c" if r["direction"]=="toward fraud" else "#32ecc71" for r in reasons]
        ))
